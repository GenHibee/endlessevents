...
const Login = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background relative overflow-hidden">
      ...
      <div className="fixed bottom-1/4 right-1/4 w-96 h-96 bg-accent/20 rounded-full blur-3xl" />
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative w-full max-w-md mx-4">
        <div className="p-8 rounded-2xl glass">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-display font-bold">Welcome to <span className="text-gradient">Endless Events</span></h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <Input id="email" type="email" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} className="mt-2" />
            </div>
            <Button type="submit" variant="gradient" className="w-full" size="lg" disabled={isLoading}>
              {isLoading ? <div className="w-4 h-4 border-2 border-primary-foreground border-t-transparent rounded-full animate-spin" /> : <><Mail className="w-4 h-4 mr-2" />Continue with Email</>}
            </Button>
          </form>
          ...
          <p className="text-xs text-center text-muted-foreground mt-6">A smart wallet will be created automatically. No crypto experience needed.</p>
        </div>
      </motion.div>
    </div>
  );
